...
with st.expander("ℹ️ Dokumentasi Model Forecasting"):
    st.markdown(
        ...
        "otomatis via grid search AIC.\n"
        "- **ETS / Holt-Winters** — exponential smoothing dengan komponen tren & "
        "musiman (seasonal_periods=12 bila data ≥24 bulan).\n"
        "- **XGBoost / Gradient Boosting** — machine learning berbasis lag features; "
        "fallback ke Gradient Boosting bila XGBoost tidak terpasang.\n"
        "- **Croston / TSB** — khusus *intermittent demand* (banyak nilai nol, "
        "mis. spare part).\n"
        "- **Auto Select Best Model** — menjalankan semua model lalu memilih MAPE "
        "terkecil.\n\n"
        "Metrik **MAE / RMSE / MAPE / sMAPE / Bias** dihitung out-of-sample "
...
def _auto_select(series, horizon: int) -> tuple[dict, pd.DataFrame, str]:
    ...
    Croston/TSB HANYA diikutkan bila data terdeteksi intermittent (banyak nol),
    karena model itu memang dirancang untuk pola tersebut.
    Return (best, comp_table, reason).
    """
    candidates = ["ARIMA", "ETS / Holt-Winters", "XGBoost / Gradient Boosting"]
    info = croston.is_intermittent_demand(series)
    ...
    best_metric = comp_sorted.iloc[0][sort_key]
    others = [str(m) for m in comp_sorted["Model"].tolist()[1:]]
    if best_key == "Croston / TSB":
        reason = (f"Data terdeteksi intermittent (ADI={info.get('adi')}, "
                  f"rasio nol={info.get('zero_ratio')}), sehingga {best_name} "
                  "diprioritaskan untuk pola demand dengan banyak nilai nol.")
    else:
        metric_txt = (f"{sort_key} terendah ({best_metric})"
                      if best_metric is not None else f"{sort_key} terbaik")
        comp_txt = (" dibanding " + ", ".join(others)) if others else ""
        reason = f"{best_name} dipilih karena {metric_txt}{comp_txt}."
    ...
    if up is not None:
        try:
            ...
            replace_old = st.checkbox(
                "Ganti seluruh demand history lama SKU ini", value=True, key="dh_replace")
            if st.button("💾 Simpan ke tabel demand_history", disabled=not can_save,
                         help=save_help):
