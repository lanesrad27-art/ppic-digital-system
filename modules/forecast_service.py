...
def save_forecast_run_with_results(run_meta: dict, rows: list[dict]) -> bool:
    ...
    dalam SATU transaksi. Bila salah satu gagal, semua di-rollback (atomic) —
    mencegah forecast_runs tersimpan tanpa forecast_results.
    """
    engine = get_engine()
    if engine is None or not rows:
        return False
    try:
        ...
        logger.error("save_forecast_run_with_results gagal: %s", e)
        return False
    ...
    try:
        with engine.begin() as conn:
            ...
            old_last_sync = (log["old_last_forecast_sync_at"]
                             if "old_last_forecast_sync_at" in log else None)
            conn.execute(text("""
                UPDATE products
                SET annual_demand = :old,
                    annual_demand_source = :src,
                    last_forecast_sync_at = :old_last,
                    updated_at = CURRENT_TIMESTAMP
                WHERE sku = :sku
...
def sync_all_forecasts(synced_by, allow_big_change: bool = False):
    """
    Sinkronkan SEMUA forecast pending sekaligus.
    - Melewati SKU yang tidak ditemukan di products.
    - Bila allow_big_change=False, melewati run dengan perubahan >50%.
    Return (ok, message, detail_list).
    """
    pending = load_pending_forecasts()
    if pending.empty:
        return False, "Tidak ada forecast pending.", []
    synced = skipped = failed = 0
    detail = []
    for _, r in pending.iterrows():
        run_id = r.get("run_id")
        sku = r.get("product_sku")
        if not bool(r.get("sku_found", True)):
            skipped += 1
            detail.append({"sku": sku, "status": "dilewati", "alasan": "SKU tidak ditemukan"})
            continue
        ok, msg = sync_forecast_to_product(run_id, synced_by, allow_big_change=allow_big_change)
        if ok:
            synced += 1
            detail.append({"sku": sku, "status": "sukses", "alasan": msg})
        elif "50%" in (msg or ""):
            skipped += 1
            detail.append({"sku": sku, "status": "dilewati", "alasan": msg})
        else:
            failed += 1
