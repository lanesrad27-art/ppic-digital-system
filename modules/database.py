...
def seed_database(seed_file: str = "seed_data.sql", schema_dir: str = "sql") -> tuple[bool, str]:
    ...
    Sebaiknya hanya dijalankan pada database kosong/baru.
    """
    engine = get_engine()
    if engine is None:
        return False, _FRIENDLY_DB_ERROR
    base = os.path.join(os.path.dirname(os.path.dirname(__file__)), schema_dir)
    path = os.path.join(base, seed_file)
    if not os.path.exists(path):
        return False, f"File seed tidak ditemukan: {seed_file}"
    try:
        with open(path, "r", encoding="utf-8") as f:
            sql_text = f.read()
        with engine.begin() as conn:
            conn.execute(text(sql_text))
        clear_cache()
        return True, "Data contoh berhasil dimuat."
    except Exception as e:  # noqa: BLE001
        logger.error("seed_database gagal: %s", e)
        return False, "Gagal memuat data contoh. Periksa seed_data.sql atau hak akses."


def get_table_counts() -> dict:
    """Ambil jumlah baris tabel utama (untuk halaman System Health)."""
    engine = get_engine()
    if engine is None:
        return {}
    tables = [
        "inventory_users", "categories", "suppliers", "products",
        "stock_transactions", "demand_history", "forecast_runs",
        "forecast_results", "forecast_sync_logs",
    ]
    counts: dict = {}
    for t in tables:
        try:
            with engine.connect() as conn:
                counts[t] = int(conn.execute(
                    text(f"SELECT COUNT(*) FROM {t}")  # nama tabel statis, bukan input user
                ).scalar() or 0)
